...
router.get("/", async (req, res) => {
  ...
  if (req.isHr) for (const b of boetes) vandaag.push({ icoon: "receipt_long", kleur: "oranje", plaat: b.kenteken, tekst: `${b.status === "nieuw" ? "Boete te bevestigen" : "Uitzondering op doorbelasten gevraagd"} · ${h.euro(b.bedrag) || "bedrag onbekend"} · ${b.bestuurder || "bestuurder onbekend"} · ${h.formatDate(b.datum)}`, pill: null, knop: { url: `/boetes/${b.id}`, tekst: b.status === "nieuw" ? "Bevestigen" : "Beoordelen" } });
