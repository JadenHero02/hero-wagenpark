...
router.get("/export.xlsx", directie, async (req, res) => {
  ...
  if (req.isHr) blad(wb, "Boetes", await db.all("SELECT b.*, v.kenteken, bs.naam AS bestuurder, u.name AS bevestigd_door_naam FROM boetes b JOIN voertuigen v ON v.id = b.voertuig_id LEFT JOIN bestuurders bs ON bs.id = b.bestuurder_id LEFT JOIN users u ON u.id = b.bevestigd_door ORDER BY b.datum DESC"), [
