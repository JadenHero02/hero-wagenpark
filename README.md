...
Mapstructuur: `server.js` (start en routes), `src/db.js` (database), `src/auth.js` (sessies en rollen), `src/mail.js` (mail via Graph, maillog), `src/taken.js` (de takenmotor: automatische taken en herinneringen, dagmail), `src/processen.js` (de vier checklists en wat de app daarin zelf doet), `src/versies.js` (versienummer en wat er per versie veranderde; vul dit aan bij elke commit die iets voor gebruikers verandert), `src/rdw.js` (open data van de RDW op kenteken: merk, model, bouwjaar, brandstof, APK-datum, kleur, catalogusprijs, WA-verzekerd, terugroepacties met tekst, APK-keuringen met gebreken; geen sleutel nodig), `src/routes/` (per onderdeel), `views/` (EJS-templates), `public/` (stijl, lettertypen, logo), `scripts/import-excel.js` (de import).

De takenmotor draait bij het opstarten en daarna elk half uur. Eén keer per dag haalt hij eerst alle kentekens langs de RDW: de APK-datum komt van de RDW, lege velden worden ingevuld, een auto die volgens de RDW niet WA-verzekerd is komt op het dashboard, en een open terugroepactie wordt een taak voor de bestuurder (met mail) die vanzelf sluit als de fabrikant herstel meldt. Onderhoud (`src/onderhoud.js`, migratie `hero_wagenpark_onderhoud`): kleine en grote beurt per auto op tijd (en op kilometers als die bekend zijn), tabel `beurten` als historie, een maand vooraf een taak en mail voor de bestuurder, na de afspraak "beurt gedaan?" met de kilometerstand. Elke automatische taak heeft een sleutel en elke herinnering een ref in het maillog, zodat niets twee keer ontstaat. Termijnen staan in de tabel `instellingen` en zijn door de admin te wijzigen. Documenten staan in de database (`document_inhoud`, tot 10 MB per bestand), niet op schijf: Railway heeft geen blijvende schijf. Het sleepvak op de documentenpagina stuurt bestanden eerst naar `/voertuigen/:id/documenten/herken`; `src/herken.js` bepaalt per bestand de soort (met `ANTHROPIC_API_KEY` via Claude, model `HERKEN_MODEL` of claude-opus-5, anders op bestandsnaam), de gebruiker bevestigt, daarna gaat het naar de gewone upload met `soort_i` en `omschrijving_i`.
